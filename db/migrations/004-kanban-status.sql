-- Migration 004: Extend status for kanban board
-- Changes status from ('open', 'done') to ('todo', 'doing', 'done')

-- SQLite doesn't support ALTER COLUMN with CHECK constraint changes
...
-- Step 2: Copy data, converting 'open' to 'todo'
...
SELECT id, project_id, title, body,
       CASE WHEN status = 'open' THEN 'todo' ELSE status END,
...
-- Step 5: Recreate FTS triggers (need to rebuild FTS too)
